...
def test_right_subtract_row_level(dummy: DataFrame) -> None:
...
def test_right_subtract_row_level_extra_row_left(
    dummy: DataFrame, dummy_extra: DataFrame
) -> None:
...
@pytest.mark.parametrize("dummy_count,dummy_extra_count", [(2, 3)])
def test_right_subtract_row_level_extra_row_right(
    dummy: DataFrame, dummy_extra: DataFrame
) -> None:
...
def test_row_level_different_row_in_left(
    dummy: DataFrame, dummy_skewed: DataFrame
) -> None:
...
@pytest.mark.parametrize("dummy_count", [2])
def test_symmetric(dummy: DataFrame) -> None:
...
def test_symmetric_different_rows_left_control(
    dummy: DataFrame, dummy_skewed: DataFrame
) -> None:
    ...
    received: DataFrame = diffit.symmetric_level(left=dummy, right=dummy_skewed).sort(
        F.col("dummy_col01"), F.col("dummy_col02")
    )
...
def test_symmetric_different_rows_left_control_on_right(
    dummy_skewed: DataFrame, dummy: DataFrame
) -> None:
    ...
    # when I check for differences at the row level
    received: DataFrame = diffit.symmetric_level(left=dummy_skewed, right=dummy).sort(
        F.col("dummy_col01"), F.col("dummy_col02")
    )
    ...
    msg = (
        "Symmetric check over different Spark DataFrames (left control on right)"
...
@pytest.mark.parametrize("dummy_count,dummy_extra_count", [(2, 3)])
def test_symmetric_extra_row_in_right(dummy: DataFrame, dummy_extra: DataFrame) -> None:
...
def test_symmetric_extra_row_in_left(dummy_extra: DataFrame, dummy: DataFrame) -> None:
...
@pytest.mark.parametrize("dummy_count", [2])
def test_symmetric_filter_left_orientation(
    dummy: DataFrame, symmetric_data_df: DataFrame
) -> None:
    ...
    received: DataFrame = diffit.symmetric_filter(
        target=dummy, symmetric=symmetric_data_df
...
def test_symmetric_filter_right_orientation(
    dummy_skewed: DataFrame, symmetric_data_df: DataFrame
) -> None:
    ...
    received: DataFrame = diffit.symmetric_filter(
        target=dummy_skewed, symmetric=symmetric_data_df, orientation="right"
    ).sort(F.col("dummy_col01"), F.col("dummy_col02"))
...
@pytest.mark.parametrize("dummy_count,dummy_skewed_count", [(2, 2)])
def test_column_level_diff(dummy: DataFrame, dummy_skewed: DataFrame) -> None:
    """Test Spark DataFrame column-level diff check."""
    # Given a set of same schema, different valued Spark SQL DataFrames
    # dummy and dummy_skewed

    # when I check for diffs at the column level
    received: Iterable[Dict[Text, Any]] = diffit.column_level_diff(
        left=dummy, right=dummy_skewed
    )
...
@pytest.mark.parametrize("dummy_count", [2])
def test_column_level_diff_with_no_diff_to_report(dummy: DataFrame) -> None:
    """Test Spark DataFrame column-level diff check: no diff to report."""
    # Given a set of different like Spark SQL DataFrames
    left = right = dummy

    # when I check for diffs at the column level
    received: Iterable[Dict[Text, Any]] = diffit.column_level_diff(left, right)
