...
def test_report_against_like_dataframes(dummy: DataFrame):
...
def test_report_against_differing_dataframes(dummy_extra: DataFrame, dummy_skewed: DataFrame):
    """Test a diffit report output.
    """
    # Given a set of like Spark SQL DataFrame schemas with different values
    ...
    # when I report on differences
    received: DataFrame = diffit.reporter.row_level(left=dummy_extra, right=dummy_skewed)\
...
@pytest.mark.parametrize('dummy_extra_count,dummy_skewed_count', [(3,2)])
def test_report_against_differing_dataframes_range_filtering(dummy_extra: DataFrame,
                                                             dummy_skewed: DataFrame):
...
@pytest.mark.parametrize('dummy_extra_count,dummy_skewed_count', [(3,2)])
def test_report_against_differing_dataframes_with_dropped_col(dummy_extra, dummy_skewed):
    ...
    # when I report on differences
    received: DataFrame = diffit.reporter.row_level(left=dummy_extra,
                                                    right=dummy_skewed,
                                                    columns_to_drop=['dummy_col02'])

    # then I should recieve the different rows across both source DataFrames
    expected = [[3, 'left']]
    msg = 'Spark DataFrame diffit report produced incorrect results'
    assert [list(row) for row in received.sort('dummy_col01').collect()] == expected, msg


@pytest.mark.parametrize('dummy_extra_count', [9])
def test_range_filter_clause(dummy_extra):
    ...
    condition = diffit.reporter.range_filter_clause(dummy_extra.schema, column_name, lower, upper)

    # and filter the original Spark SQL DataFrame
    received: DataFrame = dummy_extra.filter(condition).select(column_name)

    # then I should get a slice of the original Spark SQL DataFrame
    msg = 'Range filtered Spark SQL DataFrame slice error'
    expected = [2, 3, 4, 5]
    assert [row.dummy_col01 for row in received.orderBy('dummy_col01').collect()] == expected, msg


@pytest.mark.parametrize('dummy_extra_count', [9])
def test_range_filter_clause_lower_only(dummy_extra):
...
@pytest.mark.parametrize('dummy_extra_count', [9])
def test_range_filter_clause_upper_only(dummy_extra):
...
@pytest.mark.parametrize('dummy_extra_count', [9])
def test_is_supported_range_condition_types_integertype(dummy_extra):
    ...
    received: Text = diffit.reporter.is_supported_range_condition_types(
...
@pytest.mark.parametrize('dummy_extra_count', [9])
def test_is_supported_range_condition_types_not_supported(dummy_extra):
...
def test_grouped_rows_unique_rows(diffit_data_df: DataFrame):
    ...
    received: DataFrame = diffit.reporter.grouped_rows(diffit_data_df, column_key=column_key)
...
def test_grouped_rows_matched_rows(diffit_data_df: DataFrame):
    ...
    received: DataFrame = diffit.reporter.grouped_rows(diffit_data_df,
...
def test_distinct_rows(diffit_data_df: DataFrame):
    ...
    received: DataFrame = diffit.reporter.distinct_rows(diffit_data_df,
                                                        column_key=column_key,
                                                        diffit_ref='left')
...
def test_distinct_rows_no_match(diffit_data_df: DataFrame):
...
def test_altered_rows(diffit_data_df: DataFrame):
...
def test_altered_rows_no_match(diffit_data_df: DataFrame):
...
def test_altered_rows_column_diffs(diffit_data_df: DataFrame):
    ...
    # when I filter altered rows
    received: List[dict] = diffit.reporter.altered_rows_column_diffs(
            diff=diffit_data_df,
            column_key=column_key,
...
def test_altered_rows_column_diffs_filtered_key_value_missing(diffit_data_df: DataFrame):
